    ...
    public class ManagerSubPulse
    {
        ...
        class ProcessSet : ISerializable
        {
            ...
            internal ProcessSet(SerializationInfo info, StreamingContext context)
            {
                ...
                Dictionary<string, List<StringIdentifier>> instanceProcessors = (Dictionary<string, List<StringIdentifier>>)info.GetValue(nameof(InstanceProcessors), typeof(Dictionary<string, List<StringIdentifier>>));
            ...
            {
                Dictionary<string, List<StringIdentifier>> instanceProcessors = new Dictionary<string, List<StringIdentifier>>();
                foreach (var kvpitem in InstanceProcessors)
                {
                    string typeName = kvpitem.Key; //.TypeName;
                    instanceProcessors.Add(typeName, new List<StringIdentifier>());
